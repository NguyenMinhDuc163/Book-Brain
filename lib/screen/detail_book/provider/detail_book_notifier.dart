...
class DetailBookNotifier extends BaseNotifier {
  ...
  Future<bool> setHistoryBook({String? note, int? chapNumber}) async {
    ...
    return await execute(() async {
      ...
      historyService.updateHistory(
        ...
        readingStatus:
            chapNumber == bookDetail?.totalChapters ? "completed" : "reading",
        completionRate: rate,
        notes: note ?? "",
  ...
  Future<bool> getNoteBook({
    required int bookId,
    required int chapterId,
  }) async {
    if (!AuthHelper.isLoggedIn) return false;
    return await execute(() async {
      noteBook = await detailBookService.getNoteBook(
        bookId: bookId,
        chapterId: chapterId,
      );
      notifyListeners();
  ...
  Future<bool> saveNoteBook({
    ...
    if (!AuthHelper.isLoggedIn) return false;
    return await execute(() async {
      bool? isSUCC = await detailBookService.saveNoteBook(
        bookId: bookId,
        chapterId: chapterId,
